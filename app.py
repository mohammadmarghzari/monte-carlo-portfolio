...
# ---------- SIDEBAR ----------
st.sidebar.header("📂 بارگذاری و تنظیمات")
...
if uploaded_files:
    ...
    for file in uploaded_files:
        ...
        with st.sidebar.expander(f"⚙️ تنظیمات بیمه `{name}`"):
            insured = st.checkbox(f"📌 فعال‌سازی بیمه برای {name}", key=f"insured_{name}")
            if insured:
                loss_percent = st.number_input(f"📉 درصد ضرر معامله پوت", 0.0, 100.0, 30.0, step=0.01, key=f"loss_{name}")
                strike = st.number_input(f"🎯 قیمت اعمال پوت", 0.0, 1e6, 100.0, step=0.01, key=f"strike_{name}")
                premium = st.number_input(f"💰 قیمت قرارداد پوت", 0.0, 1e6, 5.0, step=0.01, key=f"premium_{name}")
                amount = st.number_input(f"📦 مقدار قرارداد", 0.0, 1e6, 1.0, step=0.01, key=f"amount_{name}")
                spot_price = st.number_input(f"📌 قیمت فعلی دارایی پایه", 0.0, 1e6, 100.0, step=0.01, key=f"spot_{name}")
                asset_amount = st.number_input(f"📦 مقدار دارایی پایه", 0.0, 1e6, 1.0, step=0.01, key=f"base_{name}")
                insured_assets[name] = {
    ...
    # ---------- نمایش ----------
    tabs = st.tabs(["📋 داده‌ها", "📈 پرتفو بهینه", "📉 Married Put"])
    ...
    with tabs[2]:
        for name, info in insured_assets.items():
            st.subheader(f"📉 نمودار سود و زیان Married Put - {name}")
