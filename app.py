...
# --- تابع خواندن فایل CSV و استخراج price/date
...
# --- تابع ساخت لینک دانلود فایل csv
...
# --- تبدیل داده Yahoo Finance به price/date
...
# --- محاسبه max drawdown برای یک سری قیمتی
...
# --- نمایش بازده و ریسک در بازه‌های مختلف
...
<h3>ابزار تحلیل پرتفو و بیمه (Married Put)</h3>
این ابزار برای تحلیل و بهینه‌سازی ترکیب دارایی‌های پرتفو، محاسبه پرتفوی بهینه طبق ریسک و بازده، و بررسی بیمه (Married Put) برای هر دارایی طراحی شده است. می‌توانید داده‌های خود را آپلود یا از یاهو فاینانس دانلود کنید، هر دارایی را حذف کنید، و تاثیر بیمه را دقیق ببینید.
</div>
""", unsafe_allow_html=True)

# --- بارگذاری و حذف دارایی‌ها (دانلودی/آپلودی)
...
# --- دانلود داده آنلاین از Yahoo و افزودن به لیست
...
# --- تحلیل پرتفو و محاسبه اثر بیمه بر ریسک
if st.session_state["downloaded_dfs"] or st.session_state["uploaded_dfs"]:
    ...
    # --- دقیق کردن تاثیر بیمه بر ریسک و بازده پرتفو با شبیه‌سازی مسیر بیمه‌شده
    ...
    # --- محاسبات مونت‌کارلو/ CVaR و نمایش نمودارها
    n_portfolios = 3000
    n_mc = 1000
