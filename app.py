...
def read_csv_file(file):
    try:
        ...
        # تغییر نام ستون‌ها به 'Date' و 'Price' به صورت حساس به حروف کوچک
        ...
        if 'Date' not in df.columns or 'Price' not in df.columns:
            raise ValueError("ستون‌های لازم ('Date' و 'Price') در فایل موجود نیست.")

        # تبدیل تاریخ به datetime و حذف مقادیر نامعتبر
        df['Date'] = pd.to_datetime(df['Date'], errors='coerce')
        df = df.dropna(subset=['Date'])

        # حذف کاماهای قیمت و تبدیل به عددی
...
if uploaded_files:
    ...
    for file in uploaded_files:
        ...
        # تنظیم ایندکس و ستون نام دارایی
        df = df[['Date', 'Price']].set_index('Date')
        df.columns = [name]

        # ادغام داده‌ها بر اساس تاریخ مشترک
        if prices_df.empty:
            prices_df = df
        else:
    ...
    # بازنمونه‌گیری با توجه به بازه انتخاب شده
