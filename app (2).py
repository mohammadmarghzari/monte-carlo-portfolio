...
if uploaded_files:
    ...
    for file in uploaded_files:
        ...
        try:
            ...
            # نمایش اطلاعات دیتافریم برای دیباگ
            st.write(f"اطلاعات دیتافریم برای فایل {name} قبل از تبدیل:")
            st.write(df.info())
            ...
            if df[date_col].isna().all():
                ...
                for fmt in ['%Y-%m-%d', '%d/%m/%Y', '%Y/%m/%d', '%Y-%m-%d %H:%M:%S', '%d-%m-%Y']:
                    ...
                    if not df[date_col].isna().all():
                        break
                if df[date_col].isna().all():
                    st.error(f"ستون تاریخ ({date_col}) در فایل {name} به درستی به datetime تبدیل نشد. لطفاً فرمت تاریخ رو چک کن (مثال: '2024-05-15', '15/05/2024', یا '1403-02-26').")
                    continue
            ...
            # نمایش اطلاعات دیتافریم بعد از تبدیل
            st.write(f"اطلاعات دیتافریم برای فایل {name} بعد از تبدیل:")
            st.write(df.info())
    ...
    st.write("اطلاعات دیتافریم نهایی (prices_df):")
    st.write(prices_df.info())
